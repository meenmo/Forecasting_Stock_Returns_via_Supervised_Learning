...
*Remove columns that are not contained stocks.csv I imported above, order by ticker, drop a column, and rename some varialbes;
proc sql;
	create table prices as
		select adjusted.date, adjusted.Ticker, adjusted.close from adjusted (rename=(symbol = Ticker)) 
			where Ticker in(
