...
Perhaps the most important hyperparameter for each ensemble technique is the total number of estimators. This was the first hyperparameter we tuned was done on the base decision tree model with all default settings and can be seen in Figure 3. We determined that the optimal number of estimators for each ensemble method was 150. At this point and increase in the number of estimators only leads to marginal increases in performance but leads to increased computational complexity. For all other parameters in each model, we applied a grid search. The hyperparameters we chose to evaluate were the minimum number of samples required to split a node, the max depth of the decision tree, the maximum number of features to consider for each split and the learning rate for a boosting.

\begin{figure}
  \includegraphics[height=7cm, width=8.5cm]{Plots/Comparison.png}
  \caption{{\bf Figure 3}: Selecting the Total Number of Estimators for each Ensemble Method}
\end{figure}
...
\begin{figure}[htb]
    ...
    \end{minipage}  
    \label{fig:1-2-3}
    \caption{{\bf Figure 4}: Optimal Hyperparameters for Each Algorithm}

\end{figure}
The heatmap graphics above depicted the optimal hyperparameter of each algorithm in the validation data set found through grid search utilizing the time series split with 10 folds, where the x-axis is the maximum depth of the tree, and the y-axis is the minimum samples of the tree in order to split. It can be observed in general that as max depth increase and min sample leaf decrease, the algorithm will tend to overfit which results in a decreased performance in the testing dataset. Thus we have to figure out best performing hyperparameters as deep as possible but as many as minimum sample leaves at the same time. For Adaboost, we also varied learning rate in order to penalize overfitting of the algorithms. For linear regression, there is no hyperparameter as the algorithm tries to minimize loss function. As a result., we discovered that the optimal hyper parameters for each model as follows. 
...
\bottomrule
\end{tabular}}
\caption{Optimal Hyperparametersof Each Model}
\end{table}

After optimal hyperparameters were obtained, each of the optimal models were trained with the testing dataset, of which the results are summarized in the tables below. There shows that most of the algorithm except boosting produced negative $R^2$ values which means that the algorithm is predicting worse than the mean stock returns in the testing set. Linear regression produced the worst prediction due to its complexity. However, AdaBoost algorithm performed best resulting in $R^2$ values of 0.06\%. This positive $R^2$ is hard to obtain due to the random nature of stock returns and its difficulty to predict the market. Even though 0.06 for $R^2$ cannot be told that the model functions well enough for prediction, we just highly value its positivity implying better than average stock return. 
...
\begin{figure}[h]
  \includegraphics[height=6cm, width=8cm]{MSE_R2_linear_pred.jpg}
  \caption{{\bf Figure 4}: $R^2$\& MSE of Final Test Prediction}
\end{figure}

In addition to predicting returns which might be a too difficult for a project for college students, we also took the stock return predictions and classified them into binary predictions. The binary prediction are 0 if our algorithm predicted stock returns to go down tomorrow, and 1 if the stock returns goes up tomorrow. 

Throughout the same prediction, the results of the binary classification can be summarized as confusion matrix below. We could observe that random forest algorithm performs better in this case. This means that even though random forest was one of the worst algorithm when predicting returns, it actually predicted when the stock goes up or down more correctly more often than other algorithms. The other impressive finding from the binary prediction is that all of all four algorithm have the over 50\% accuracy. However, considering the fact that a single percent matters in practice where billions of billions dollar transact every day, these result are not trivial.

\begin{figure}[h]
  \includegraphics[height=8.5cm, width=8.5cm]{Plots/ConfusionMatrix.png}
  \caption{{\bf Figure 5}: Confusion Matrices for Binary Prediction}
\end{figure}
...
\bottomrule
\end{tabular}}
\caption{Binary Prediction Accuracy}
\end{table}


Additionally, we also utilized min/max scaler in order to obtain probabilities of stock returns going up or down tomorrow. This results in a ROC plot where the area under the curve is a metric to classify the best algorithm. However, when utilizing this metric, bagging performs best.
It is hard to define the reason why bagging produced the best figure in ROC due to its complexity and small margin of each AUC. Anyhow, all four algorithms produced better prediction than random, or 50\%, which is good considering the fact that randomness of stock price. We also made sure that all the testing set have balanced 0 and 1. 

\begin{figure}[h]
  \includegraphics[height=6cm, width=8cm]{Plots/ROC_Curve_1.png}
  \caption{{\bf Figure 6}: ROC Curve}
\end{figure}
...
{\bf Jonathan Santoso (Model Ninja)}: Created lagged stock return features, helped out in the selection of machine learning algorithm, and hyperparameter tuning via grid-search. Moreover, Jonathan was involved in determining cross-validation splits and also helped out with the interpretation of results.
